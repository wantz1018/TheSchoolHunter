    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if (resultSet_1.next()){
                ...
                //修改接收方积分
                sql = "update users set balance = ? where u_id = ?";
                PreStatement.execute(sql, new String[]{String.valueOf(Double.valueOf(receive_balance).intValue() + Double.valueOf(points).intValue() * (Double.parseDouble(rank) / 5)), receive_id});
